...
5. Le dépôt utilise un **`Dockerfile`** multi-étapes (voir `railway.toml` → `builder = DOCKERFILE`) : l’image finale contient `dist/` et les dépendances prod. Si tu avais un Dockerfile personnalisé vide dans l’UI Railway, supprime-le pour utiliser celui du dépôt.
